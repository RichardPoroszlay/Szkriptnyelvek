    ...
    if "not" in s:
        if "bad" in s:
            ...
            if not_index < bad_index:
                return "good".join([s[:not_index],s[bad_index+3:]]) # +3-mal úgy mozgatjuk az intervallumot, hogy a "bad" eltűnik
        return s
    return s
...
def front_back(a, b):
    ...
    if len(a) % 2 == 0:
        ...
        a_eleje = a[0:half_size]
        a_vege = a[half_size:]
    else:
        half_size = len(a)//2
        a_eleje = a[0:half_size+1]
        a_vege = a[half_size+1:]
    
    b_eleje = ""
    b_vege = ""
    if len(b) % 2 == 0:
        half_size = int(len(b)/2)
        b_eleje = b[0:half_size]
        b_vege = b[half_size:]
    else:
        half_size = len(b)//2
        b_eleje = b[0:half_size+1]
        b_vege = b[half_size+1:]
